    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl shadow-sm animate-fade-in-up">
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-2 flex items-center gap-2">
          <Database className="w-5 h-5 text-blue-600 dark:text-blue-500" /> Course 5: System Logs & Forensics
        </h2>
      ...
      <div className="space-y-2">
        <Accordion title="5.1 Parsing the Raw Log" icon={Search} isOpen={openSection === 0} onClick={() => toggleSection(0)}>
          <div className="space-y-4 text-slate-700 dark:text-slate-300 text-sm sm:text-base leading-relaxed">
            ...
            <div className="bg-slate-50 dark:bg-slate-950 border-l-4 border-l-red-500 border-y border-r border-slate-200 dark:border-slate-800 rounded-lg p-4 font-mono text-xs overflow-x-auto text-slate-800 dark:text-slate-300">
            ...
            <ul className="list-disc pl-5 space-y-2 mt-4 text-slate-600 dark:text-slate-400 text-sm sm:text-base">
        ...
        <Accordion title="5.2 Forensic Automation in Python" icon={FileCode2} isOpen={openSection === 1} onClick={() => toggleSection(1)}>
          <div className="space-y-4 text-slate-700 dark:text-slate-300 text-sm sm:text-base leading-relaxed">
            ...
            <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg p-4 font-mono text-xs overflow-x-auto text-slate-800 dark:text-slate-300 mt-2">
              <span className="text-indigo-600 dark:text-indigo-400">import</span> pandas <span className="text-indigo-600 dark:text-indigo-400">as</span> pd<br/><br/>
              <span className="text-slate-500"># Load historic logs</span><br/>
              df = pd.read_csv(<span className="text-emerald-600 dark:text-emerald-400">'wids_logs.csv'</span>)<br/><br/>
              <span className="text-slate-500"># Find the most targeted devices (top victims)</span><br/>
              top_targets = df[df[<span className="text-emerald-600 dark:text-emerald-400">'attack_type'</span>] == <span className="text-emerald-600 dark:text-emerald-400">'DEAUTH'</span>][<span className="text-emerald-600 dark:text-emerald-400">'target_mac'</span>].value_counts()<br/>
              <span className="text-blue-600 dark:text-blue-400">print</span>(top_targets.head(<span className="text-blue-600 dark:text-blue-400">5</span>))<br/><br/>
              <span className="text-slate-500"># Calculate average signal strength of attacker</span><br/>
              avg_rssi = df[df[<span className="text-emerald-600 dark:text-emerald-400">'attack_type'</span>] == <span className="text-emerald-600 dark:text-emerald-400">'ROGUE_AP'</span>][<span className="text-emerald-600 dark:text-emerald-400">'rssi'</span>].mean()<br/>
              <span className="text-blue-600 dark:text-blue-400">print</span>(<span className="text-emerald-600 dark:text-emerald-400">{`f"Average Rogue AP RSSI: {avg_rssi} dBm"`}</span>)
            </div>
          </div>
        </Accordion>
        ...
        <Accordion title="5.3 False Positive Tuning" icon={Sliders} isOpen={openSection === 2} onClick={() => toggleSection(2)}>
          <div className="space-y-4 text-slate-700 dark:text-slate-300 text-sm sm:text-base leading-relaxed">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mt-4">
              <div className="bg-slate-50 dark:bg-slate-900/50 p-5 rounded-xl border border-slate-200 dark:border-slate-800 border-l-4 border-l-emerald-500">
                <h4 className="font-semibold text-slate-800 dark:text-slate-200 mb-2 text-sm sm:text-base">Normal Behavior</h4>
                <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
                  A device sending 1 or 2 Deauth frames when walking out of range or shutting down is completely normal. 
                </p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-900/50 p-5 rounded-xl border border-slate-200 dark:border-slate-800 border-l-4 border-l-red-500">
                <h4 className="font-semibold text-slate-800 dark:text-slate-200 mb-2 text-sm sm:text-base">Malicious Behavior</h4>
                <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
